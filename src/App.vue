    ...
    methods: {
        ...
        getFiltredDevs: function () {
            ...
            escopo.filtredDevs = devs.filter(dev => {
                let semFiltro = !escopo.search && escopo.linguagem.length === 0
                let nomesBatendo = !escopo.search || escopo.getFilterCharacters(dev.name).indexOf(escopo.getFilterCharacters(escopo.search)) >= 0
...
.card {
    box-shadow: 0 10px 10px 0.5px grey;
    border-radius: 10px;
    display: grid;
